...
const Site = (props:IProps) => {
    ...
    useEffect(() => {
        if(result.data){
            const { setSites : { status, location, token, errors } } = result.data
            if (status === 201) {
        ...
        <AbstractComponent>
            ...
            <SiteContainer>
                <div style={{ position: 'relative', width: '100%', maxWidth: `${common.size.mobileWidth}px` }}>
                    ...
                    <Fab style={{ position: 'absolute', bottom: '5px', right: '-25px', zIndex: 1 }} color="primary" aria-label="add" onClick={onClickAdd}>
                        <AddIcon />
                    </Fab>
                </div>
